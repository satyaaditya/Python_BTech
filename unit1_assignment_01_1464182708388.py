...
def get_odds_desc(count):
    ...
    """
    return list(reversed(range(1,2*count,2)))
...
def get_multiples_desc(number, count):
    ...
    """
    return sorted(range(number,number*(count+1),number),reverse=True)
...
def ascii_distance(first, second):
    ...
    """
    return abs(ord(second)-ord(first))  # use 2 builtin functions to get your work done.
...
def get_binary(number):
    ...
    """
    a= bin(number)
    return a[2:] # use a builtin and slice the result ...
