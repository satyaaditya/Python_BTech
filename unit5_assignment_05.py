...
def prune_either_or(sentence):
    ...
    if sentence==' ':
        return ' '
    min = sentence.find('either')
    max=sentence.rfind('either')
    or_quo=sentence.find('or')
    if ((min==0)):
        return sentence
    if (((min>-1)&(or_quo>-1))):
            if (sentence.index('either') > sentence.index('or')):
                return sentence
    if ((min==-1)|(or_quo==-1)):
            return sentence
    if not check_whether_either(sentence):
            return sentence
    else:
            if min!=max:
                sentence=sentence[:max-1]
            else:
                sentence = sentence.rpartition('or')
                sentence = sentence[0]
...
def check_whether_either(sentence):
    i=0;spelling='';
    while i<len(sentence):
        if sentence[i]==' ':
            if sentence.find("either")==1:
                return False
            if spelling!="either":
                spelling=''
            else:
