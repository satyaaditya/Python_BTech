...
def are_anagrams(first, second):
    if ((first==None)|(second==None)):
        return False
    first= (''.join(i for i in first if i.isalnum())).lower()
    second=(''.join(i for i in second if i.isalnum())).lower()
...
def test_are_anagrams_student():
    ...
    assert True == are_anagrams("Buckethead", "Death Cube K',''',':';")
    assert True == are_anagrams(" ", " ")
    assert False == are_anagrams("sat", "")
    assert False == are_anagrams(None, "satya")
