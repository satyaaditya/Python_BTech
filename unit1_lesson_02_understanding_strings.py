...
def test_strpings_can_be_indexed():
    string = "Hello"
    assert "H" == string[0]
    assert "e" == string[1]
    assert "l" == string[2]
...
def test_string_membership():
     #is there a precedence issue here or something else?
    assert True or False == 'a' in 'apple'
...
def rotate_right(input, count):
    ...
    """
    return input[len(input)-count:len(input)]+input[:len(input)-count] # replace __ with the code
...
- better understanding of slicing and concatenation
-precedence of operations
-in python we can use negative indices unlike c
"""
