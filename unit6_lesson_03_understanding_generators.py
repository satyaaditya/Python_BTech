...
def test_range_allocates_memory():
    try:
        for item in range(100 * (10**9)):
            if item%5 == 1:
                break
    except OverflowError :  # what error do you get when you allocate 100 billion ints?
        assert True
...
def test_evens():
    ...
    for num in xrange(1000):
        assert 2*num == gen.next()

    for num in xrange(1000):
        assert 2*(num+1000) == gen.next()
...
def fizz_buzz():
    ...
    while True:
        ...
        i = 0
        if cur%3==0:
            ret_tup=ret_tup.__add__(('fizz',))
            i+=1
        if cur%5==0:
            ret_tup=ret_tup.__add__(('buzz',))
            i+=1
        if i==0:
            ret_tup=ret_tup.__add__((None,))
        yield ret_tup
        cur+=1
...
-what are genrators,how would a next function be called
-difference between generators and iterators
-and yield which is different from return and stubborn nature of generators
"""
